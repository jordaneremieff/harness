...
8. **Continue or read back.** Read `references/iteration.md` when the operator requests a sustained sequence, supplies or asks for an outcome, or before triaging a reply or making any continuation decision. It owns reply acquisition, round limits, stop conditions, and the outcome format. A single press that ends at delivery does not need it.
