...
export const CORRECTION_TASK =
	"If a task is in progress, use this assessment to correct its affected reasoning, plans, and artifacts within the existing authorization, then continue that task in this turn. Do not stop at the assessment or ask for permission to resume work the operator already authorized. If no correction is warranted, continue the task unchanged rather than manufacture one.";
export const JUDGMENT_AUTHORITY =
	"This invocation does not authorize corpus edits or approve corpus proposals, and it grants no authority for new surfaces, destructive acts, publication, or credential use.";
...
export function judgmentPrompt({ action, hint }: JudgmentRequest): string {
	return [
		...
		action === "check"
			? `Check the subject for Pillars alignment. Explain the consequential matches or tensions with evidence and recommend any needed change. Use judgment about depth and form; do not turn this into a recital or scorecard. ${CORRECTION_TASK} If no task is in progress, the assessment completes this request; do not invent follow-on work. If a genuine operator decision or an authority you were not granted blocks part of the task, name that exact boundary, hold only the actions that depend on it, and complete the rest. ${JUDGMENT_AUTHORITY}`
			: action === "review"
