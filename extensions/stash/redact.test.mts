...
describe("redactSecrets", () => {
	...
	it("redacts JWTs and bearer tokens", () => {
		const jwt = "eyJhbGciO" + "iJSUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIxMjM0NTY3ODkwIn0.SflKxwRJSMeKKF2QT4fwpMeJf36POk6yJV_adQssw5c";
	...
	it("redacts labeled credentials with compound and spaced keys", () => {
		...
		assert.equal(redactSecrets("STRIPE_SECRET_KEY=sk_live_a" + "bcdefghijklmnopqrstuvwxyz"), `STRIPE_SECRET_KEY=${REDACTED}`);
	...
	it("redacts PGP armor and tokens wrapped across lines", () => {
		...
		const wrappedJwt = "eyJhbGciO" + "iJSUzI1NiJ9.eyJzdWIiOiIxMjM0NTY3\nODkwfQ.SflKxwRJSMeKKF2QT4fwpMeJf36POk6yJV_adQssw5c";
		assert.equal(redactSecrets(wrappedJwt), REDACTED);
	});

	it("keeps weak-label prose and short token values intact", () => {
		assert.equal(redactSecrets("The parser token: IdentifierName remains valid"), "The parser token: IdentifierName remains valid");
	...
	it("redacts DSN userinfo passwords for any scheme", () => {
		assert.equal(redactSecrets("postgres://postgres:password12345@localhost:5432/db"), "postgres://postgres:[REDACTED]@localhost:5432/db");
		assert.equal(redactSecrets("mongodb://user:p@ssw0rd123@cluster.example.com:27017/db"), "mongodb://user:[REDACTED]@cluster.example.com:27017/db");
	...
	it("keeps URL values under assignment keys intact", () => {
		assert.equal(redactSecrets("token: https://example.com/oauth/callback"), "token: https://example.com/oauth/callback");
	...
	it("redacts assignment values in json, bare, and quoted forms", () => {
		...
		assert.equal(redactSecrets("refresh_token = eyJhbGciO" + "iJSUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIxIn0.abc"), `refresh_token = ${REDACTED}`);
	});

	it("redacts private key blocks and URL userinfo passwords", () => {
		const key = "-----BEGIN RSA PRIVATE KEY-----\nMIIEpAIBAAKCAQEA\n-----END RSA PRIVATE KEY-----";
		assert.equal(redactSecrets(`key material:\n${key}\nend`), `key material:\n${REDACTED}\nend`);
		assert.equal(redactSecrets("see https://user:p4ssw0rd@example.com/path"), "see https://user:[REDACTED]@example.com/path");
	...
	it("preserves non-secret content", () => {
		...
		assert.equal(redactSecrets("commit 9fceb02d0ae533e0b443ae44d7f495a0761d3a8e"), "commit 9fceb02d0ae533e0b443ae44d7f495a0761d3a8e");
		assert.equal(redactSecrets("uuid 9fceb02d-0ae5-33e0-b443-ae44d7f495a0"), "uuid 9fceb02d-0ae5-33e0-b443-ae44d7f495a0");
		...
		assert.equal(redactSecrets("mysk-ant-oa" + "t01-abcdefghijklmnopqrstuvwx"), "mysk-ant-oa" + "t01-abcdefghijklmnopqrstuvwx");
		assert.equal(redactSecrets("the Bearer scheme is documented in rfc6750"), "the Bearer scheme is documented in rfc6750");
...
describe("redactPayload", () => {
	it("redacts every string field", () => {
		...
		const all = [out.title, out.summary, ...(out.decisions ?? []), ...(out.openLoops ?? []), ...(out.nextActions ?? []), ...(out.files ?? []), ...(out.tags ?? [])].join("\n");
